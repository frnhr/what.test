...
class Product(models.Model):
    ...
    def generate_fake(cls, n: int) -> list[int]:
        ...
        def _fake_price() -> float:
            ...
            Using the price_formats to avoid having 90% of prices having the
            same number of digits.
            """
            return float(
                fake.numerify(
                    fake.random_element(
                        faker.providers.currency.en_US.Provider.price_formats,
                    ),
                ).replace(",", ""),
            )
        ...
        def _fake_name() -> str:
            ...
            I can personally confirm that reading generated values is a very fun
            way to waste time.
            """
            ...
            _min_word_len = 3
            long_words = list(
                filter(lambda word: len(word) > _min_word_len, words),
            )  # remove "a", "the", etc.
        ...
        def _random_stock() -> int:
            """
            Generate a random stock number, with more chances of having a low
            stock.
            """
            return fake.random_int(
                min=0,
                max=10 ** fake.random_int(min=1, max=4),
            )
